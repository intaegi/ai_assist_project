...
`resolution_notices`에 성공 메시지를 저장하고, 재생성을 선택하면 추가 자료를 반영한 새 버전을 만든다.

`チェックリストをAIで確認`은 첨부문서, 현재 폼, 서버 검증 결과를 AI에 전달하여 각 항목을
`verified`, `action_required`, `not_verifiable`로 분류한다. 서버의 미해결 오류·경고가 AI 판정보다
우선하므로, 관련 경고가 남아 있는 항목은 AI가 확인 완료로 반환해도 `action_required`로 보정한다.
최종 체크박스 선택은 사용자가 수행한다.
...
재생성 또는 수정 후에는 Streamlit의 기존 폼 `session_state`를 제거한 뒤 화면을 다시 그린다.
따라서 서버의 최신 값이 저장됐지만 화면에는 이전 값이 남는 현상을 방지한다.
