...
`FastAPI`は文書抽出、検索知識のBlob保存、Indexer対象の整理、検索、生成呼出しを制御する。
...
UI-00は設定担当者が基準資料を登録するために使用する。UI-02は利用者が次の行動を
判断しやすいように、`生成結果表示`、`結果確認`、`AIチャット`の3領域で構成する。
`生成結果表示`では原本書類とAI生成決裁フォームを左右に配置する。`結果確認`では
不足・注意、チェックリスト、必要書類を要約カードで先に表示し、生成内容や書類比較表などの
詳細は初期状態で閉じた展開領域に表示する。`AIチャット`では修正対象のセレクトボックスを
廃止し、自然文の依頼を決裁案全体の再確認・再生成として処理する。類似決裁と修正履歴は、
必要なときだけ開く`詳細情報`の展開領域に表示する。
...
├──────────────────────┴─────────────────────────────┴──────────────────────────┤
│ 結果確認                                                                 │
│ 要約カード: 不足・注意 / チェックリスト / 必要書類                       │
│ [要約・書類比較・不足確認を表示 ▾]                                      │
│   生成内容 / 書類間比較 / 不足・注意詳細 / チェックリスト詳細            │
│ [詳細情報 ▾] 類似決裁・修正履歴                                         │
├─────────────────────────────────────────────────────────────────────────────┤
│ AIチャット                                                               │
│ AIへの依頼内容 [本文を3文以内にしてください.........................]    │
│ [AIに相談・修正] 生成結果と修正チャット履歴に反映します。               │
└─────────────────────────────────────────────────────────────────────────────┘
```
