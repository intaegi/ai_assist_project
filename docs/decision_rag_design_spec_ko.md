...
flowchart LR
    ...
    Prep -->|Blob 저장・메타데이터 부여| Blob
    Blob -->|Data source + Skillset + Indexer| Search
...
`FastAPI`는 문서 추출, 검색 지식의 Blob 저장, 인덱서 실행 대상 정리, 검색과 생성 호출을 제어한다.
...
UI-00은 설정 담당자가 기준 자료를 등록할 때 사용한다. UI-02는 사용자가 다음 행동을 쉽게
판단할 수 있도록 `생성결과표시`, `결과확인`, `AI채팅`의 3영역으로 구성한다.
`생성결과표시`에는 원본 문서와 AI 생성 결재 폼을 좌우로 배치한다. `결과확인`에는
부족·주의, 체크리스트, 필요서류를 요약 카드로 먼저 표시하고, 생성 요약과 문서 비교표 등
상세내용은 기본 접힘 영역에 표시한다. `AI채팅`은 수정 대상 셀렉트박스를 제거하고,
자연어 지시를 결재안 전체 재검토 요청으로 처리한다. 유사 결재와 수정 이력은 일반 사용자가
필요할 때만 여는 `상세정보` 확장 영역에 표시한다.
...
| 수정 채팅 | `AI채팅` 영역에서 수정 대상 선택 없이 자연어로 입력하고, 결재안 전체를 재검토・갱신 |
